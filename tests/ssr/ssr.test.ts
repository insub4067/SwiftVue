...
const Kitchen = defineComponent({
  setup() {
    ...
    return () => h(VStack, { spacing: 8 }, () => [
      ...
      h(Image, { src: '/a.png', alt: 'A', resizable: true, contentMode: 'fill' }),
      h(AsyncImage, { url: '/b.png', alt: 'B' }),
      h(Form, { spacing: 16 }, () => h(Text, () => 'form-row')),
      h(DatePicker, { modelValue: '2026-08-09', displayedComponents: 'date', label: 'Date' }),
      h(Menu, { label: 'More', actions: [{ label: 'Rename' }, { label: 'Delete', role: 'destructive' }] }),
    ])
  },
})
